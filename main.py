...
def scan_file(file_path, report_data):
    found_in_file = 0
    triggered_lines = set()
...
def start_scanning(target_path, custom_exclude=None, json_output=None):
    ...
    if os.path.isdir(target_path):
        for root, dirs, files in os.walk(target_path):
            ...
            for file in files:
                ...
                total_secrets += scan_file(full_path, report_data)
    else:
        total_secrets += scan_file(target_path, report_data)
...
if __name__ == "__main__":
    ...
    start_scanning(args.path, custom_exclude=args.exclude, json_output=args.json)